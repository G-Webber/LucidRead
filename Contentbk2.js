...
var is_tag_str = "";
var is_tag_Scritp = 0;
var is_Comment_Singleton = false;
var not_tag_str = "";
let tagMax = 20;
let popped = "";
...
for (var i = 0; i < html.length; i++){
	...
	//	console.log('not a tag: ' + this_tag + ': :'+ this_tag.match(htmlTagRe));
	//	}
    if ((brace_open > -1) && (tag_prop == 0)){   // we are probably in a tag not confirmed
		if(is_tag_Scritp){
			
			if(!(this_tag.toLowerCase() == ("/script".substring(0, this_tag.length)))){  //(this_tag.length == 7) && (!(this_tag.toLowerCase() == "/script"))) {
				brace_open = -1;
				
			}
		}
		else if (this_tag.substring(0, 3) == "!--"){   //the tag name is available now !!!! should stop looking for it // special case for ill formed comments
            new_html += character;
			tag_prop = 1;
			is_Comment_Singleton = true;
			console.log("comment Opened");
			continue;
            }
        else if ((character == ' ')){   //the tag name is available now !!!! should stop looking for it // special case for ill formed comments
            new_html += character;
			tag_prop = 1;
			continue;
            }
		...
		else if (!is_tag_Scritp){
			console.log('nevern,');
		}
    }
	if (is_Comment_Singleton && (html.substring(i-2,i+1 ) == '-->')){
		is_Comment_Singleton = false;
		brace_open  = -1;
		tag_prop = 0;
		console.log("comment closed");
		console.log("char "+ character );
		console.log(" substring " + html.substring(i-2,i+1 ))
		console.log(" substr " + html.substr(i-2,3));
		console.log(" char at " + html[i]);
		console.log("this_tag " + this_tag);
	}
    else if ((character == '>')&&(brace_open > -1)){
		if (is_Comment_Singleton) console.log("comment NOT closed");
        brace_open= -1;
		tag_prop = 0;

        if (this_tag[0] == '/'){
			popped = current_tags.pop();
			console.log(" popped :" + current_tags);
			if (popped.toLowerCase() == "script"){
				is_tag_Scritp = 0;
			}
				if (!(popped == this_tag.substr(1))) console.log("tags don't match!!!" + popped + ",  ,/" + this_tag.substr(1) + ". ." + html.substring(i, i+20));
				
				if ((this_tag.substr(1) in ignore) && (is_tag_Scritp == 0)) {
				ignorance_level--;
				}
        ...
        else if ((!(this_tag[0] == '!'))&&(!(this_tag in singletons))&& (is_tag_Scritp == 0) && (!(this_tag == ""))) { //(this_tag in allHtml) some seems redundand as is done when tag_prop is set
			if (!(this_tag in allHtml)) console.log(" not a tag :" + this_tag);
            current_tags.push(this_tag);
			console.log(" pushed :" + current_tags);
    ...
    else if ((character == '<')&& (!is_Comment_Singleton)){ //could be the start of a tag
	this_tag = "";
